...
<body>
<div class="transparent LogoWrapper-sc-1duRon-1 dEKxIf">
    <a class="sc-1I1iYs-1 gXtlRg" href=#>
        <svg width="133" height="26" xmlns="http://www.w3.org/2000/svg">
            ...
            <g>
                <text xml:space="preserve" text-anchor="start" font-family="Helvetica, Arial, sans-serif" font-size="24"
                      id="svg_1" y="21.6" x="0" stroke-width="0" stroke="#000" fill="#426373">Monkey Doc</text>
            </g>
        </svg>
    </a>
</div>
<div id="app">
    ...
    <div class="now_login"><span class="switch-tips">已有账号？请</span>
        <button class="sm-button  sc-1n784rm-0 sfCUt" @click="login" type="default">登录</button>
    </div>
    <div id="app1">
        <label>用户名</label><input type="text" placeholder="请设置用户名" v-model="userInfo.username" class="input"
                                 @blur="a"><br>
        <span class="tiShi">{{ tiShi.tishi1 }}</span><br>
        <label>设置密码</label><input type="password" placeholder="请设置登录密码" v-model="userInfo.password1" class="input"
                                  @blur="b"><br>
        ...
        <label style=" position: relative;left:15px;height: 10px">性别</label>
        <input type="radio" id="male" value="male" v-model="userInfo.sex" style="margin-left:0px;">
        <label for="male" style=" position: relative;right:20px;height: 10px">男</label>
        <input type="radio" id="female" value="female" v-model="userInfo.sex">
        <label for="female" style=" position: relative;right:20px;height: 10px">女</label><br><br>
        <span class="tiShi" style="top: -10px;position: relative">{{ tiShi.tishi9}}</span><br>
        <label>手机号</label><input type="text" placeholder="请输入手机号码" v-model="userInfo.phonenum" class="input"
                                 @blur="e"><br>
        <span class="tiShi">{{ tiShi.tishi5}}</span><br>
        <label>公司地址</label><textarea rows="5" cols="26" placeholder="请输入您的公司地址" v-model="userInfo.address"
                                     @blur="f"></textarea><br>
        <span class="tiShi">{{ tiShi.tishi6}}</span><br>
        <div class="checkbox"><input type="checkbox" style="margin-right: 10px;" v-model="userInfo.checkbox">阅读并接受<a
                href="">《xxx用户协议》</a></div>
    ...
    var vm = new Vue({
        ...
        methods: {
            ...
            thisAjax() {
                ...
                axios.post("http://localhost:8080/MonkeyDocsStruts_war_exploded/business/userLogon.action", {
                    ...
                    res => {
                        ...
                        } else if (res.headers.responsemsg == "phone_has_been_logon") {
                            this.tiShi.tishi5 = "手机号已注册";
                            return;
                        } else if (res.headers.responsemsg == "email_has_been_logon") {
                            this.tiShi.tishi7 = "电子邮箱已注册";
                            return;
                        } else if (res.headers.responsemsg == "both_have_been_logon") {
                            this.tiShi.tishi5 = "手机号已注册";
                            this.tiShi.tishi7 = "电子邮箱已注册";
                            return;
                        }
            ...
            d() {
                ...
                if (!this.userInfo.name) {
                    this.tiShi.tishi4 = '姓名不能为空';
                    this.isReturn = true;
                } else if (!r.test(this.userInfo.name)) {
                    this.tiShi.tishi4 = '姓名格式不正确,请使用中国汉字';
                    this.isReturn = true;
                }
            },
            //手机号
            e() {
                ...
                if (!this.userInfo.phonenum) {
                    ...
                    this.isReturn = true;
                } else if (!n.test(this.userInfo.phonenum)) {
                    this.tiShi.tishi5 = '手机号码格式不正确';
                    this.isReturn = true;
                }
            },
            f() {
                ...
                if (!this.userInfo.address) {
                    this.tiShi.tishi6 = '请输入您的公司或学校地址';
                    this.isReturn = true;
                }
            },
            g() {
                ...
                if (!this.userInfo.email) {
                    this.tiShi.tishi7 = '电子邮箱不能为空';
                    this.isReturn = true;
                } else if (!r.test(this.userInfo.email)) {
                    this.tiShi.tishi7 = '电子邮箱格式不正确';
                    this.isReturn = true;
                }
            },
            h() {
                ...
                if (!this.userInfo.idcard) {
                    this.tiShi.tishi8 = '身份证号码不能为空';
                    this.isReturn = true;
                } else if (!r.test(this.userInfo.idcard)) {
                    this.tiShi.tishi8 = '身份证号码格式不正确';
                    this.isReturn = true;
                }
            },
            flag() {
                ...
                if (!this.userInfo.name) {
                    this.tiShi.tishi4 = '姓名不能为空';
                    this.isReturn = true;
                } else if (!e.test(this.userInfo.name)) {
                    this.tiShi.tishi4 = '姓名格式不正确,请使用中国汉字';
                    this.isReturn = true;
                }
                //手机号
